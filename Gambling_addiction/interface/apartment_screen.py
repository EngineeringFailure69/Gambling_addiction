...
def apartment_screen():
    ...
    tier_1_apartment = apartment_class.apartment(0, 100000, 300, "assets\\icons\\apartment_icons\\tier_1_apartment.png")
    const.real_estate_list.append(tier_1_apartment)
    tier_2_apartment = apartment_class.apartment(1, 300000, 1000, "assets\\icons\\apartment_icons\\tier_2_apartment.png")
    const.real_estate_list.append(tier_2_apartment)
    tier_3_apartment = apartment_class.apartment(2, 600000, 2000, "assets\\icons\\apartment_icons\\tier_3_apartment.png")
    const.real_estate_list.append(tier_3_apartment)
    tier_4_apartment = apartment_class.apartment(3, 1000000, 3000, "assets\\icons\\apartment_icons\\tier_4_apartment.png")
    ...
    while running:
        ...
        if month != month_old:
            const.balance -= const.current_rent_price
            month_old = month

        real_estate_button_rect, inventory_button_rect = draw_functions.draw_apatment(const.screen, mouse_pos, screen_width, screen_height, font, clock, events, apartment, const.current_rent_price, const.renting_apartment_index)
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if len(rect_list_apartment) > 0:
                    ...
                    if rect_list_apartment[1].collidepoint(mouse_pos): #Rent
                        if const.balance < apartment.rent_price:
                            ...
                            const.balance -= apartment.rent_price
                            const.renting_apartment_index = apartment.index
                            draw_apartment_list = False
                            const.current_rent_price = apartment.rent_price
